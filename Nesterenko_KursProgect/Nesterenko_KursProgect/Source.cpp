...
tryAgain1:
    ...
    tryAgain2:
        cout << "Введіть розмір дошки (5 - "
            << maxBoardSize - 4 << "): ";
        cin >> boardSize;
        if (boardSize < 5 || boardSize > maxBoardSize - 4) //перевірка
        {
            InputError;
            cin.clear();
            while (cin.get() != '\n');
    ...
    tryAgain3:
        cout << "Введіть розмір дошки (" << n << "-"
            << maxBoardSize - 4 << "): ";
        cin >> boardSize;
        if (boardSize < n || boardSize > maxBoardSize - 4) //перевірка
        {
            InputError;
            cin.clear();
            while (cin.get() != '\n');
    ...
    while (successWords != n)
    {
        ...
        for (int stop = 0, word = 1; word < n && stop == 0; word++)
        {
            ...
            if (bestCross != 0)
            {
                ...
                stop = 1;    // Встановлюємо прапор зупинки циклу генерації слів
        }
    }
